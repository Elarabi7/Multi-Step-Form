...
function validateForm() {
    ...
    for (let i = 0; i < formInputs.length; i++) {
        if (!formInputs[i].value) {
            ...
            formInputs[i].style.borderColor = "hsl(354, 84%, 57%)"; // Set outline to Strawberry Red
            findLabel(formInputs[i]).nextElementSibling.style.display = "block";
        } else {
            valid = true;
            formInputs[i].classList.remove("err");
            formInputs[i].style.borderColor = "hsl(231, 11%, 63%)"; // Reset to Cool Gray
